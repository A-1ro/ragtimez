  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        '- "topic": English description of the chosen topic (1 sentence). SINGLE-PRODUCT RULE (CRITICAL): The topic MUST describe exactly ONE product, service, or announcement. Topics that combine two or more products with "and", "plus", or "/" are STRICTLY FORBIDDEN and cause article rejection — for example, "Gemma 4 and Strands Evals SDK" is FORBIDDEN because it names two unrelated products. Select only the single most interesting product or announcement and name it alone.\n' +
