  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "   MARKETING/ADVERTISING EXCLUSION (ALWAYS REJECT, no exceptions): Never select a topic whose primary source is a TV commercial, advertisement, brand marketing campaign, promotional video, or PR/brand-image piece — even if it name-drops AI products or features (e.g. a Google ad that shows Google Docs and Gemini being used in a fictional scenario contains ZERO API names, version numbers, or configuration details, and covering it would force fabricating a technical article about a commercial). If every entry in the news list is marketing/advertising content with no separate technical announcement, entry (c) above applies: skip all of them and pick the least-hype item available rather than inventing technical substance for an ad.\n" +
