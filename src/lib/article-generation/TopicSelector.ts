  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "   GLOSSARY/TERMINOLOGY REJECTION CRITERIA: Do NOT select a topic if ALL of the following apply: (a) the primary source is a glossary, terminology guide, or conceptual framework article that defines terms without providing code examples, runnable commands, API endpoints, or version-specific details; (b) the topic cannot yield at least ONE concrete engineering action the reader can take TODAY — a command to run, an API call to make, a config key to set, or an official repository URL to clone/try. Glossary-only sources that only define concepts without any implementable artifact are insufficient for a technical deep-dive. Only select a glossary topic if you can identify a concrete entry point engineers can act on immediately from the source text.\n" +
