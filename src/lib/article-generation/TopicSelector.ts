  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "CITATION ACCOUNTABILITY RULE: Only include an entry in `indices` if you can immediately name at least one specific technical detail from that entry — a named system, version number, API name, benchmark figure, or architectural fact — that will appear as a cited fact in the article body. If an entry is only 'contextually related' or 'same industry' but has no citable technical specifics about the chosen topic, EXCLUDE it. Entries that appear in indices but are never cited in the article body pollute the frontmatter sources list and mislead readers. Fewer indices with guaranteed citation is strictly better than more indices with uncertain relevance.\n" +
