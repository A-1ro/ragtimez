  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        '- "indices": array of 1-based entry numbers that are DIRECTLY relevant to this topic. STRICT LIMIT: include at most 5 entries. STRICT RELEVANCE RULE: only include an entry if it contains technical details, announcements, or official documentation SPECIFICALLY about the chosen topic — not merely about the same time period or industry. FORBIDDEN entries (must be excluded regardless): entries about a different company\'s financials (funding rounds, equity deals, layoffs, earnings reports), entries about unrelated products or workforce events even from the same company, general market commentary or opinion pieces without technical substance. FORBIDDEN SOURCE TYPES (must always be excluded regardless of topic): ASMR content, meditation, wellness, or lifestyle pages (identified by: ASMR in title/URL, or words like "manifest", "relax", "sleep" in title); hackathon project writeups for a domain unrelated to the chosen topic (e.g., a CNC manufacturing entry when topic is voice interfaces); any source where the only overlap with the chosen topic is a single common English word (e.g., both contain "whisper") but subject matter is unrelated. CONCRETE EXAMPLES OF FORBIDDEN INCLUSIONS: if the topic is a medical AI framework, an entry titled "Nvidia commits $40B to equity deals" or "Oracle workers negotiate severance" MUST be excluded — they are factually unrelated even if they are recent news. If an entry is about a different product made by the same company as the chosen topic, it MUST be excluded. When in doubt, EXCLUDE the entry. Fewer indices with perfect relevance is strictly better than more indices with mixed topics. PRODUCT FEATURE SPECIFICITY TEST (apply before inclusion): When the topic is a specific product feature or launch (e.g., "ChatGPT personal finance feature", "Azure AI Foundry new capability"), each candidate entry must pass this test — its title or first paragraph must explicitly name that specific feature or product. Entries about the same company but covering a different product, legal case, podcast episode, or org event FAIL this test and MUST be excluded. Entries from different companies discussing unrelated domains (energy, real estate, cloud storage unrelated to the topic) FAIL this test regardless of publication date. EXAMPLES OF PRODUCT FEATURE SPECIFICITY FAILURES: if the topic is "ChatGPT personal finance feature", a TechCrunch podcast about an OpenAI legal trial MUST be excluded (legal event, not the feature); a TechCrunch article about Silicon Valley energy needs MUST be excluded (unrelated industry); an AWS article about S3 knowledge bases MUST be excluded (different company, different feature); an OpenAI blog post about an unrelated product like Codex or DALL-E MUST be excluded (different product from same company).\n' +
