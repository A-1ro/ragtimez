  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "   MANDATORY REJECTION CRITERIA: Reject a topic if ALL of the following apply: (a) the primary source is a trend/opinion article with NO implementation details such as API names, version numbers, or architectural specifics; (b) no other selected [Source] index provides concrete implementation details about THIS SAME topic; (c) covering the topic would require the LLM to rely on pre-training knowledge to fill technical sections. When rejecting, set indices to [] in the output and choose a different topic.\n" +
