  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "   MARKETING CONTENT EXCLUSION (CRITICAL): Advertisements, TV/video commercials, brand campaign videos, and marketing announcements that describe a company's image, a creative campaign, or a hypothetical/fictional scenario — without disclosing any specific API, SDK, model name, version number, or architecture — ALWAYS satisfy rejection criterion (a) above, even if they name-drop a real product (e.g. a commercial that mentions 'Gemini' or 'Google Workspace' with no technical detail is still marketing content, not a technical topic).\n" +
