  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "   CONTENT EXISTENCE RULE (CRITICAL — enforced before finalizing): The exact topic name or a SPECIFIC technical aspect of it (feature name, API name, version, benchmark) MUST appear in the TITLE or SUMMARY of at least one of the entries you list in `indices`. If you cannot point to a specific sentence in a listed entry that directly covers your chosen topic, REJECT this topic and choose a different one whose content is explicitly present in the provided entries. Writing an article based on general knowledge rather than the provided entries is forbidden.\n" +
