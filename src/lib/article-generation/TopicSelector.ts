  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        '- "indices": array of 1-based entry numbers that are DIRECTLY relevant to this topic. STRICT LIMIT: include at most 5 entries. Only include entries that contain technical details, announcements, or official documentation specifically about the chosen topic. If an entry mentions the same company but covers a different product or feature, it MUST be excluded. Do NOT include tangentially related entries (e.g., general opinion pieces, unrelated product pages from the same company, community forum posts about different features). Fewer entries with higher relevance is always better than many entries with mixed relevance.\n' +
