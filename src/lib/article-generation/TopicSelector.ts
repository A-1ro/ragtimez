  ...
  async select(input: {
    ...
    const raw = await this.llmClient.generateText({
      ...
      system:
        ...
        "MANDATORY PRE-OUTPUT SELF-CHECK (you MUST perform this before writing the JSON): For EACH index you intend to include, answer ALL three questions: (A) Is the SUBJECT of this entry the SAME product or service as the chosen topic — not just the same company, same time period, or same broad industry? (B) Does this entry contain at least one concrete fact (version number, API name, architecture detail, official announcement) that would appear verbatim in the article body about the chosen topic? (C) Does this entry pass ALL FORBIDDEN ENTRIES and FORBIDDEN SOURCE TYPES checks listed above? If ANY answer is NO for an entry, remove that index immediately before outputting. A different product by the same company, a competitor's unrelated announcement, a funding/acquisition item, or a community event MUST be removed. Fewer indices with perfect relevance is strictly better than more indices with mixed topics.\n\n" +
