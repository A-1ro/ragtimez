...
:::admin-note[A-1ro]
This article contains several significant errors.

【Product Confusion】
Microsoft Discovery and Azure AI Foundry's Deep Research are separate products.
While both are offered on the same Microsoft Foundry platform, neither is built
on top of the other. Discovery is an R&D platform aimed at scientists, powered
by its own Discovery Engine and a graph-based knowledge engine.

【Code Errors】
The `azure.ai.foundry` module and `DeepResearchAgent` class shown in the code
example do not exist. The actual package available is `azure-ai-projects`, and
Deep Research is not provided as a standalone agent class — it is offered as a
"tool" within the Agents SDK. Additionally, the specified `gpt-4-turbo` model
is not supported; the actual implementation combines `o3-deep-research` (for
research execution) with `gpt-4o` (for intent clarification).

【Timeline Errors】
Deep Research has been available in Public Preview since July 2025. What was
expanded to enterprise preview this April was Discovery, not Deep Research.
Discovery itself was originally announced at Microsoft Build in May 2025.
